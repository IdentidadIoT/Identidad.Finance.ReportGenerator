...
# ===========================
config = init_config()
BASE_URL = cfg.get_parameter('General', 'BASE_URL')
...
def post_raw_originate_sms(start, end, billing_cycle):
...
def post_originate_sms_custom_gmt(start, end, billing_cycle):
    url = f"sms/RawOriginateSms/CustomGmt?billing_cycle={billing_cycle}"
    data = {"StartDate": start, "EndDate": end}
    post_endpoint(url, data, "RawOriginateSmsCustomGmt")
...
#0 2 * * * /usr/bin/python3 /opt/pythonapps/Identidad.Finance.ReportGenerator/worker_reportsms.py